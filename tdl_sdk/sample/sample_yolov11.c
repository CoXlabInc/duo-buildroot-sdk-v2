...
int main(int argc, char *argv[]) {
  ...
  if (argc != 3) {
    printf("Usage: %s <yolov8 model path> <input image path>\n", argv[0]);
    printf("yolov8 model path: Path to yolov8 model cvimodel.\n");
  ...
  printf("]\n");
#ifndef NO_OPENCV
  CVI_TDL_ShowDetectionBox(&obj_meta, argv[2], "./yolo11_output.jpg");
#endif 
